...
- This repo is **`Veltara-Works/pharlux-site`** — the marketing + docs site at `pharlux.com`. **Two-stack reality**: `main` is currently a single static HTML page (`index.html` + `_headers` + structured-data JSON-LD), deployed via **Cloudflare Pages auto-build** on push to `main` (`git push` IS the deploy). The `tier2-docusaurus` branch carries the in-flight migration to **Docusaurus 3.x + TypeScript**; cutover needs coordinated build-config + merge to avoid a prod outage. Infer the actual stack from the target branch's working tree (presence of `package.json` / `docusaurus.config.ts`), not from this file alone.
- **Brand and GEO/SEO are policy-bound, not Copilot-reviewable**: marketing claims, brand voice, AI-crawler allowlist, and GEO/SEO posture are governed by Veltara Works internal policy docs not visible in this repo. Don't review brand strategy itself; only flag concrete diffs that look policy-violating: unverifiable performance / feature claims, removal of `Organization` or `SoftwareApplication` JSON-LD, AI-crawler allowlist changes in `robots.txt` without justification, or brand-voice violations (see below).
- **No technical/operational leaks**: this is public-facing marketing. Never let a PR introduce any production JWKS endpoint URL, revoke webhook payload shape, per-customer secret format, internal infrastructure addresses, or enterprise-only feature names that haven't been publicly announced.
...
- **Build / accessibility / performance basics — stack-aware**:
  - On `main` (static HTML): `index.html` must validate; every image needs `alt` text; no broken internal links; OG-image / favicon paths must resolve; structured-data JSON-LD must remain syntactically valid.
  - On `tier2-docusaurus` (Docusaurus 3.x + TypeScript): `npm run build` must pass with zero broken-link warnings (Docusaurus surfaces these); every image still needs alt text; flag PRs that bundle large unoptimised images (Lighthouse-grade page weight is a real concern).
  - Either way, a broken link or missing alt is a marketing-surface defect that ships to prod on the next `main` push.
